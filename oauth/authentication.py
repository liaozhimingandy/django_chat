...
class AuthBearer(HttpBearer):
    def authenticate(self, request, token):
        ...
        # 校验jwt token并解析数据
        jwt_decode = verify_jwt_token(token, grant_type="client_credential"
        if request.resolver_match.view_name == "api-1.0.0:refresh_token" else "access_token")
        ...
        if (request.resolver_match.view_name != "api-1.0.0:refresh_token"
            ...
            raise AuthenticationError({"message": "please use access_token!"})
