...
class Comment(models.Model):
    ...
    pid = models.IntegerField("父评论ID", db_comment="父评论ID", help_text="父评论ID",
                              null=True, db_default=None)
    content = models.TextField("内容", db_comment="评论内容", help_text="评论内容")
    uid = models.CharField(max_length=36,verbose_name="评论者ID", help_text="评论者ID", db_comment="评论者ID")
    app_id = models.CharField(max_length=36, verbose_name="所属应用ID", db_comment="所属应用ID", help_text="所属应用ID",
                              db_index=True)
    post_id = models.CharField(max_length=36, verbose_name="内容ID", db_comment="内容ID", help_text="内容ID", db_index=True)
